NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "shortName": "Grid",
      "type": "function",
      "keywords": "add added additional all alter an and any api appended are array as asc ascending assign assigntypes be behavior being block build buildcolumndefsfromdata buildcolumns buildstyles by called calls chance class col coldef column columnbuilder columnbuilders columndefinition columndefs columns columnsprocessor contains count created creates currently data defined definitions desc descending direction directions directive each eached either element etc exclude existing expected false first for from function further getcolumn getcolumnsorting getrow given grid gridcolumn gridoptions gridrow has having id identified if in instance into is it iterate its load long main maintain map methods modified modifyrows name needed newrawdata not object objects of on one only optional optionally options or pass per populates process processes processor processrowbuilders promise properties property prototype provided reference register registercolumnbuilder registered registerrowbuilder registerrowsprocessor registerstylecomputation remove removerowsprocessor removes renderable reset resetcolumnsorting resetting resolved resources return returns row rowbuilder rowbuilders rowentity rowequality rows set sort sortcolumn sorted sorting state stylecomputation supplies that the this through to type types ui unsorted updated uses using value viewmodel when which will with"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "type": "overview",
      "keywords": "$apply $http $scope active address aftercelledit age all allow angular any app as available be boolean booleans by called can cell celleditablecondition cellnav cellscope checkbox city class coldef column columndef columndefs columns compiled content controller css custom data default definitions determine displayname double-click edit editable editablecelltemplate edited editing editor editors enablecelledit f2 false feature fields focus following for function get grid gridapi gridoptions has height html id if in index invoke invoked is isactive js json key lastcelledited length main mainctrl mode module msg name newvalue ng-controller non-boolean non-navigable not null number oldvalue on only onregisterapi option options or other overview provided returns row rowentity rows scope set should soon start success templatecache templates text than that the to true tutorial type typing ui ui-grid ui-grid-edit url use used using valid var via when width will with you your"
    },
    {
      "section": "tutorial",
      "id": "6_hidden_grids",
